...
def default_configs() -> Dict[Type[OpView], ChiselOpConfig]:
    ...
    return {
        ...
        # globalId; post-op reads sub-program outputs by globalId. See
        # _subprogram_pre_op / _subprogram_post_op. no_golden stays True
        # because there is no isolated per-op golden function - the golden
        # comes transitively from the sub-program; this keeps the ops in
        # get_op_names_no_golden() for the per-op unit-test skip lists.
