...
void run(const ::tt::target::ttnn::MoeComputeOp *op, ProgramContext &context) {
  ...
  std::optional<::ttnn::Tensor> optionalOutput;
  if (op->optional_output_tensor() != nullptr) {
    optionalOutput =
        tensorPool.getTTNNTensorAndValidate(op->optional_output_tensor());
  }

  std::optional<::ttnn::GlobalSemaphore> crossDeviceSemaphore;
  if (op->cross_device_semaphore() != nullptr) {
  ...
  std::optional<::ttnn::CoreRangeSet> muxCoreRangeSet;
  if (op->mux_core_range_set() != nullptr) {
    muxCoreRangeSet = ::tt::runtime::ttnn::utils::toTTNNCoreRangeSet(
        *op->mux_core_range_set());
  }
  ...
  std::optional<::ttnn::experimental::prim::detail::MoEActivationFunction>
      activation = toMetalActivation(op->activation_function());
  ...
  std::vector<::ttnn::Tensor> results = ::ttnn::experimental::moe_compute(
      ...
      op->has_bias(), op->cluster_axis(), topology, numLinks, muxCoreRangeSet,
      outputMemoryConfig, optionalOutput, crossDeviceSemaphore, activation);
