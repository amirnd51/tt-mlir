...
// CHECK-LABEL: func.func @test_noc_write_one_packet_trid_and_noc_nonconst
// CHECK-SAME: (%[[TRID:.*]]: i32, %[[NOC:.*]]: i8)
func.func @test_noc_write_one_packet_trid_and_noc_nonconst(%trid: i32, %noc: i8) {
  // CHECK: %[[C0:.*]] = arith.constant 0 : index
  // CHECK: %[[SRC:.*]] = arith.constant 256 : i32
  // CHECK: %[[DST:.*]] = arith.constant 512 : i32
  // CHECK: %[[SIZE:.*]] = arith.constant 128 : i32
  // CHECK: ttkernel.noc_async_write_one_packet_with_trid(%[[SRC]], core[%[[C0]], %[[C0]]], %[[DST]], %[[SIZE]], %[[TRID]], %[[NOC]]) : (i32, index, index, i32, i32, i32, i8) -> ()
  %c0 = arith.constant 0 : index
  %src = arith.constant 256 : i32
  %dst = arith.constant 512 : i32
  %size = arith.constant 128 : i32
  ttkernel.noc_async_write_one_packet_with_trid(%src, core[%c0, %c0], %dst, %size, %trid, %noc) : (i32, index, index, i32, i32, i32, i8) -> ()
  return
}
...
// CHECK-LABEL: func.func @test_noc_write_one_packet_trid_with_implicit_noc
// CHECK-SAME: (%[[TRID:.*]]: i32)
func.func @test_noc_write_one_packet_trid_with_implicit_noc(%trid: i32) {
  // CHECK: %[[C0:.*]] = arith.constant 0 : index
  // CHECK: %[[SRC:.*]] = arith.constant 256 : i32
  // CHECK: %[[DST:.*]] = arith.constant 512 : i32
  // CHECK: %[[SIZE:.*]] = arith.constant 128 : i32
  // CHECK: ttkernel.noc_async_write_one_packet_with_trid(%[[SRC]], core[%[[C0]], %[[C0]]], %[[DST]], %[[SIZE]], %[[TRID]]) : (i32, index, index, i32, i32, i32) -> ()
  %c0 = arith.constant 0 : index
  %src = arith.constant 256 : i32
  %dst = arith.constant 512 : i32
  %size = arith.constant 128 : i32
  ttkernel.noc_async_write_one_packet_with_trid(%src, core[%c0, %c0], %dst, %size, %trid) : (i32, index, index, i32, i32, i32) -> ()
  return
}
...
func.func @test_noc_inline_dw_write(%x: index, %y: index, %dst: i32, %val: i32, %be: i8, %noc: i8) {
  // CHECK: ttkernel.noc_inline_dw_write(core[%[[X]], %[[Y]]], %[[DST]], %[[VAL]], %[[BE]], %[[NOC]]) : (index, index, i32, i32, i8, i8) -> ()
  ttkernel.noc_inline_dw_write(core[%x, %y], %dst, %val, %be, %noc) : (index, index, i32, i32, i8, i8) -> ()
  return
}
