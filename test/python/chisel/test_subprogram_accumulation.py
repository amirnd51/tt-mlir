...
def _submit_passthrough(device, fbb, device_inputs, program_index: int = 0):
    ...
    outputs = tt_runtime.runtime.submit(
        device, fbb, program_index, list(device_inputs)
    )
    tt_runtime.runtime.wait(outputs)
    return outputs


def _program_pool_records(records):
    return [
        r
        for r in records
        if r.check == "golden_promoted"
        and r.payload.source == chisel.GoldenPromotionSource.PROGRAM_POOL
    ]
...
def test_func_call_chains_through_subprogram(device, tmp_path):
    ...
    callee_names = [
        fbb.get_program_name(i)
        for i in range(1, fbb.get_num_programs())
    ]
    ...
    pool_records = _program_pool_records(records)
    assert pool_records, (
        "expected source=program_pool records on the func.call chain; "
        f"promotions seen: "
    ...
    assert callee_arg_pool, (
        "expected at least one source=program_pool record from the callee "
    ...
    parent_call_pool = [
        ...
        if r.program_name == parent_name and r.op == "func.call"
    ]
    assert parent_call_pool, (
        "expected source=program_pool record on the parent's func.call output; "
...
def test_load_cached_cache_miss_chains_through_subprogram(device, tmp_path):
    ...
    with chisel.session(
        ...
        outs, _ = _submit_with_layout_convert(
    ...
    pool_records = _program_pool_records(records)
    assert pool_records, (
        "expected source=program_pool records from the load_cached chain; "
        f"promotions seen: "
        f"{[(r.op, r.ssa, r.payload.source, r.program_name) for r in records if r.check == 'golden_promoted']}"
    )
    ...
    assert parent_loadcached_pool, (
        "expected source=program_pool record on the parent's ttcore.load_cached "
...
def test_load_cached_cache_hit_chains_through_subprogram(device, tmp_path):
    ...
    post-op must still see source=program_pool on the second run."""
    ...
    run2_pool_records = _program_pool_records(records_run2)
    assert run2_pool_records, (
        "expected source=program_pool records on run 2 (cache hit); "
        f"run-2 promotions: "
    ...
    assert run2_loadcached_pool, (
        "run 2 (cache hit): expected source=program_pool on the parent's "
