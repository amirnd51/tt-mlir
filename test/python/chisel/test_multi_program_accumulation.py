...
pool (source="program_pool"), rather than re-seeding from the device.
...
def test_program_pool_chains_a_to_b(device, tmp_path):
    """Output of program A is fed as input to program B; chisel should
    record at least one PROGRAM_POOL promotion on B's first op."""
    ...
    assert pool_records, (
        "expected at least one golden_promoted record with source='program_pool' "
        "on program B's first op; promotions seen: "
        f"{[(r.op, r.ssa, r.payload.source) for r in records if r.check == 'golden_promoted']}"
    )


def test_program_pool_chains_a_to_a_to_a(device, tmp_path):
    """Re-running the same program three times, each run consuming the prior
    run's output. Runs 2 and 3 should each see at least one PROGRAM_POOL
    ...
    # Runs 2 and 3 should each contribute at least one program_pool promotion
    # on the first op (the chained input arg).
    pool_records = _program_pool_records(records)
    assert len(pool_records) >= 2, (
        f"expected at least 2 program_pool promotions across 3 runs, got "
