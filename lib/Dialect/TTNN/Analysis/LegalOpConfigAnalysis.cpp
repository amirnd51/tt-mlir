...
void LegalOpConfigAnalysis::fillOpSpecificAttrs() {
  llvm::TypeSwitch<Operation *>(op)
      ...
      .Case<ttnn::Conv3dOp>([&](auto convOp) {
        ...
        Conv3dConfigGenerator generator(&convOp, baseConfig, conv3dSearchSpace,
                                        filterOut);
        ...
        if (generator.searchDone()) {
          generatedConfigs.push_back(baseConfig);
        } else {
          while (Conv3dConfigAttr configAttr = generator.getNextConfig()) {
            generatedConfigs.push_back(configAttr);
          }
        }
