...
struct DotGeneralToMatmulConversionPattern
  ...
  matchAndRewrite(ttir::DotGeneralOp op, OpAdaptor adaptor,
    ...
    // the dot_general is a pure outer product, which is just a broadcast
    // elementwise multiply. Lowering that to ttir.matmul routes through
    // tt-metal's matmul, whose hifi4 path splits the f32 operands into bf16
    // hi/lo pairs and leaves large products (e.g. RoPE frequencies in the
    // thousands) with only ~bf16 absolute precision -- which then destroys a
    // downstream sin/cos. A broadcast multiply keeps full f32 precision.
    // lhsMatmulInput is [..., M, 1] and rhsMatmulInput is [..., 1, N], so they
    // broadcast-multiply to the matmul destination shape [..., M, N].
