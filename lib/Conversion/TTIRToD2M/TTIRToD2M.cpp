...
class D2MConstantOpRewriter : public OpConversionPattern<ttir::ConstantOp> {
  ...
  matchAndRewrite(ttir::ConstantOp op, OpAdaptor adaptor,
    ...
    // The value attr's type may not match unencodedTy if the original
    // ttir.constant had its own RankedTensorType encoding. Check shape
    // + element type compatibility; mismatches would fail downstream.
